...
# PART_ADDR_PATH = '../ext_data/corpus/nbs_address_part_for_train.txt'
PART_ADDR_PATH = '../ext_data/corpus/nbs_address_word_entry_for_train.txt'
...
# address_list = pd.read_csv('../ext_data/corpus/nbs_address_part_for_train.txt',header=None).iloc[:, 0].tolist()
...
def get_name_image(image_w = 110,image_h = 40):
    ...
    card_info['addr'] = "".join(random.sample(address_list, address_len))[:12*3]  # 最多3行
    ...
    for col in ['addr']:
        ...
        for i in range(math.ceil(len(card_info[col]) / max_line)):
            adddress_line.append([card_info[col][i * max_line:(i + 1) * max_line]])
    ...
    # , 68: 68 + 250]
    return target.convert('L').crop([68 + pand_w, 130 + pand_h, 68 + 250 + pand_w, 130 + 105 + pand_h]),adddress_line,card_info['uuid']
...
def ThreadCardMaker(maker_count = 100, t_name="t1"):
    ...
    # 标签文件夹
    file = open(DATA_PATH + "/out_txt/address_lable_by_maker_{0}.txt".format(t_name), 'w', encoding='utf-8')
    text_panding = 1
    ...
    for idx in range(maker_count):
        ...
        for i,addr in enumerate(adddress_line):
            ...
            img.crop([0, 12 + 25*i - text_panding, line_width, (12 + 25*(i+1)) + text_panding]).save(DATA_PATH + '/out_img/{0}'.format(save_file))
            file.write("{0} {1}".format(save_file[:-4], addr[0]) + "\n")
