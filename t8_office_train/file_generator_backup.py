...
class FileImageDataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, data_size, corpus_file, train_dir, train_label ,df_train_dir, df_train_label ,
                 max_label_length = 12, batch_size=32, image_size=[60, 540]):
    ...
    def __getitem__(self, index):
        ...
        # # Find list of IDs
        # batch_datas = [self.label_list[k] for k in indexes]
    ...
    def data_generation(self,batch_index):
        ...
        # 随机抽取 df训练集35% 放入batch size内
        batch_list = list(range(self.batch_size))
        batch_list = random.sample(batch_list,int(self.batch_size * 0.35))
        ...
        for i, idx in enumerate(batch_index):
            ...
            new_img[:img_resize.size[1],:min(img_resize.size[0],600)] = np.array(img_resize, 'f')[:,:min(img_resize.size[0],600)]

            x[i] = np.expand_dims(new_img / 255.0 - 0.5, axis=2)
            # x[i] = np.expand_dims( np.array(img_resize, 'f') / 255.0 - 0.5, axis=2)
