...
# 保存生成的训练文件的位置
DATA_PATH = 'D:/projects/data/ccf2019_ocr/month_l2'
...
def get_nation_image(image_w = 110,image_h = 40):
    ...
    return target.convert('L').crop([139 + pand_w, 102 + pand_h, 139 + 52 + pand_w, 102 + 40 + pand_h]), (int(card_info['birthday_m']) - 1), card_info['uuid']
...
def ThreadCardMaker(maker_count = 100, t_name="t1"):
    ...
    # 标签文件夹
    file = open(DATA_PATH + "/out_txt/month_lable_by_maker_{0}.txt".format(t_name), 'w', encoding='utf-8')
    text_panding = 1
