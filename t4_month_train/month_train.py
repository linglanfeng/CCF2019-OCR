...
# 源路径
# src_path = "E:/projects/python/DF_idcard_all/temp/month"
src_path = "D:/projects/data/ccf2019_ocr/month_l2"

# 训练权重日期版本（不要重复）
DATE_VER = '1019'+ '_month'

# model_path = 'train_model/month_weights/month_weights-net-0074-0.00000054.h5' # 扩展自生成1w数据 继续训练
# model_path = 'train_model/month_weights/month_mix_ext1w_weights-net-0023-0.00836428.h5'# 中断后继续训练
model_path = WEIGHTS_PATH + '/month_mix_ext1w_weights-net-0043-0.00359372.h5' # 添加120w数据，重新训练
start_epoch = 0
...
for path,dir_list,file_list in g:
    for dir in dir_list:
        ...
        for img_name in child_imglist:
            img_label_dict[img_name] = labels[int(dir)-1]

# ======================= ImageMaker 训练数据=========================
maker_train_img = pd.read_csv(src_path + '/train_maker_label.txt',sep=" ", header=None, names=['file', 'month_idx'])
for idx, row in maker_train_img.iterrows():
    file_path = '{0}/{1}.jpg'.format(src_path + '/out_img',row['file'])
    imglist.append(file_path)
    img_label_dict[file_path] = labels[int(row['month_idx'])]
