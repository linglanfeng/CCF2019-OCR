...
# 保存生成的训练文件的位置
DATA_PATH = 'D:/projects/data/ccf2019_ocr/year_l2'
...
def get_nation_image(image_w = 110,image_h = 40):
    ...
    # , 68:68 + 70]
    return target.convert('L').crop([75 + pand_w, 102 + pand_h, 75 + 70 + pand_w, 102 + 40 + pand_h]), (int(card_info['birthday_y']) - MIN_START_YEAR), card_info['uuid']
...
def ThreadCardMaker(maker_count = 100, t_name="t1"):
    ...
    # 标签文件夹
    file = open(DATA_PATH + "/out_txt/yaer_lable_by_maker_{0}.txt".format(t_name), 'w', encoding='utf-8')
    text_panding = 1
...
if __name__ == '__main__':
    ...
    thread_make_count = 20 * W # 1013日再增加数据试试效果
