...
# 保存生成的训练文件的位置
DATA_PATH = 'D:/projects/data/ccf2019_ocr/nation_l2'
...
def get_nation_image(image_w = 110,image_h = 40):
    ...
    # 民族编号
    # nationality_idx = random.choice(range(56))
    # nationality_idx = random.choice([11,33,41]) # 回,哈萨克,拉祜
    # nationality_idx = random.choice([1,11,24, 33, 41])  # 塔吉克,回,侗,哈萨克,拉祜
    nationality_idx = random.choice([30,11,21])  # 朝鲜,回,景颇


    card_info['nationality'] = nation_list[nationality_idx]
    ...
    # , 170: 170 + 120]
    return target.convert('L').crop([170 + pand_w, 70 + pand_h, 170 + 120 + pand_w, 70 + 40 + pand_h]), nationality_idx, card_info['uuid']
...
def ThreadCardMaker(maker_count = 100, t_name="t1"):
    ...
    # 标签文件夹
    file = open(DATA_PATH + "/out_txt/nation_lable_by_maker_{0}.txt".format(t_name), 'w', encoding='utf-8')
    text_panding = 1
...
if __name__ == '__main__':
    ...
    thread_make_count = 2 * W # 1013日再增加数据试试效果
