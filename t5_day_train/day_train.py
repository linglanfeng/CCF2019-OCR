...
# 训练权重保存路径
WEIGHTS_PATH = '../x_train_model/day_weights'

# 源路径
# src_path = "E:/projects/python/DF_idcard_all/temp/day"
src_path = "D:/projects/data/ccf2019_ocr/day_l2"

# 训练权重日期版本（不要重复）
DATE_VER = '1019'+ '_day'

# model_path = 'train_model/day_weights/day_mix_weights-net-0100-0.00000629.h5' # 扩大自生成数据1w再训练
model_path = WEIGHTS_PATH + '/day_mix_ext1w_weights-net-0077-0.00052851.h5' # 添加120w数据，重新训练
start_epoch = 0
...
# ======================= ImageMaker 训练数据=========================
maker_train_img = pd.read_csv(src_path + '/train_maker_label.txt',sep=" ", header=None, names=['file', 'day_idx'])
for idx, row in maker_train_img.iterrows():
    file_path = '{0}/{1}.jpg'.format(src_path + '/out_img',row['file'])
    imglist.append(file_path)
    img_label_dict[file_path] = labels[int(row['day_idx'])]
