...
def get_name_image(image_w = 110,image_h = 40):
    ...
    return target.convert('L').crop([70 + pand_w, 38 + pand_h, 70 + 110 + pand_w, 38 + 40 + pand_h]), card_info['name'].replace(' ',''), card_info['uuid']
...
def ThreadCardMaker(maker_count = 100, t_name="t1"):
    ...
    # 标签文件夹
    file = open(DATA_PATH + "/out_txt/name_lable_by_maker_{0}.txt".format(t_name), 'w', encoding='utf-8')
    text_panding = 1
...
if __name__ == '__main__':
    ...
    thread_make_count = 100 * W # 1013日再增加数据试试效果
